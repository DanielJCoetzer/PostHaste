...
public class WolfMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Chase Player
            desiredDestination = player.GetComponent<Rigidbody2D>().position - rb.position;
        }
        else if(!atHouse) {
            // Move Normally
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        if (other.gameObject == player) {
            playerFound = true;
            player.GetComponent<ManageNeeds>().isAtWolf = true;
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other) {
        if (other.gameObject == player) {
            playerFound = false;
            player.GetComponent<ManageNeeds>().isAtWolf = false;
        }
    }
